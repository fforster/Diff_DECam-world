...
# output directory
indir = '/media/fforster_fondecyt_2Tb_2014/DATA/DuPont/%s' % obsdate
outdir = "%s/OUT" % indir
...
# file calibration and catalogue generation
if dodetrend:

    # correct images and run sextractor

    print "\nApplying bias and flat field corrections to raw images...\n"
    
    for i in sorted(files):

        if i[0:3] != 'ccd' or i[8:12] != 'fits' or i[-4:len(i)] != 'fits':
            continue

        filename = "%s/%s" % (indir, i)

        ifile = int(i[3:7])
        print i

        obs = fits.open(filename)
        header = obs[0].header
        data = obs[0].data
    
        filteri = header['FILTER']
        obj = header['OBJECT']

        if obj.lower() != supernova.lower() or filter != filteri:
            continue

        # output file
        filename = "%s/%s_%s_%04i.fits" % (outdir, supernova, filter, ifile)

        # transpose and save
        NAXIS1 = header['NAXIS1']
        NAXIS2 = header['NAXIS2']
        header.update('NAXIS1', NAXIS2)
        header.update('NAXIS2', NAXIS1)
        del header['VGAUGE']
        del header['IGAUGE']
        del header['HA']
        data = data.transpose()
        data = data[::-1, ::-1]
        fixed = fits.PrimaryHDU(data = float32(data), header = header)
        fixed.writeto(filename, clobber = True)

        print "\n\n", filename, i, ifile, filter, obj

        # run sextractor
        background = "%s/%s_%s_%04i_background-%03i.fits" % (outdir, supernova, filter, ifile, backsize)
        catalogue = "%s/%s_%s_%04i.fits-catalogue.dat" % (outdir, supernova, filter, ifile)
        command = "sex %s -CATALOG_NAME %s -BACK_SIZE %i -CHECKIMAGE_TYPE BACKGROUND -CHECKIMAGE_NAME %s -VERBOSE_TYPE QUIET" % (filename, catalogue, backsize, background)
        print command
        os.system(command)
        ...
        if doplot:
            fig, ax = plt.subplots(1, 3, figsize = (15, 17))
            fig.subplots_adjust(wspace = 0, hspace = 0)
            
            ax[0].scatter(x[flag <= 2], y[flag <= 2], marker = 'o', facecolors = 'none', color = 'white', s = 20 * r, lw = 0.5)
            ax[1].scatter(x[flag <= 2], y[flag <= 2], marker = 'o', facecolors = 'none', color = 'white',  s = 20 * r, lw = 0.5)
            ax[2].scatter(x[flag <= 2], y[flag <= 2], marker = 'o', facecolors = 'none', color = 'white', s = 20 * r, lw = 0.5)
            
            ax[0].scatter(x[flag <= 4], y[flag <= 4], marker = 'o', facecolors = 'none', color = 'y', s = 20 * r, lw = 0.5)
            ax[1].scatter(x[flag <= 4], y[flag <= 4], marker = 'o', facecolors = 'none', color = 'y', s = 20 * r, lw = 0.5)
            ax[2].scatter(x[flag <= 4], y[flag <= 4], marker = 'o', facecolors = 'none', color = 'y', s = 20 * r, lw = 0.5)
            
            ax[1].scatter(x[flag > 4], y[flag > 4], marker = 'o', facecolors = 'none', color = 'r', s = 20 * r, lw = 0.5)
            ax[2].scatter(x[flag > 4], y[flag > 4], marker = 'o', facecolors = 'none', color = 'r', s = 20 * r, lw = 0.5)
            ax[0].scatter(x[flag > 4], y[flag > 4], marker = 'o', facecolors = 'none', color = 'r', s = 20 * r, lw = 0.5)
            ...
            plt.savefig("%s/%s_%s_%04i_nosky-%03i.png" % (outdir, obj, filter, ifile, backsize), bbox_inches = 'tight', dpi = 150)
...
def intersect(x1, x2, y1, y2, deltax, deltay, npix):
    ...
    print "Refining intersection after linear transformation"

    print len(x1new)
    # now refine trying linear transformation with the previous points
    trans = findlineartransformation(x1new, y1new, x2new, y2new)

    if trans != None:
        (a11, a12, b1, a21, a22, b2) = trans

        # apply linear transformation and try new intersection
        x1t = b1 + a11 * x1 + a12 * y1
        y1t = b2 + a21 * x1 + a22 * y1
        ...
        for isource in range(len(x2)):
            ...
            distx = x2[isource] - x1t
            disty = y2[isource] - y1t
            dist = distx * distx + disty * disty
            idx = np.argmin(dist)
...
if domosaic:
    ...
    for i in sorted(files):
        ...
        filteri = header['FILTER']
        obj = header['OBJECT']
        (yyyy, mm, dd) = map(int, header['UT-DATE'].split('-'))
        (hh, mmm, ss) = map(int, header['UT-TIME'].split(':'))
        MJD = DateTime(yyyy, mm, dd, hh, mmm, ss).mjd
        ...
        fitsnew = "%s/%s_%s_%04i.fits" % (outdir, supernova, filter, ifile)
        ...
        (x, y, flux, e_flux, r, flag) = np.loadtxt("%s/%s_%s_%04i.fits-catalogue.dat" % (outdir, supernova, filter, ifile), usecols = (1, 2, 5, 6, 8, 9)).transpose()
        ...
        if doplot:
            ax.scatter(x[flag <= 4] / npix + ibest, y[flag <= 4] / npix + jbest, marker = 'o', facecolors = 'none', color = 'b', s = 10, lw = 0.2)
            plt.savefig("TMP/%04i_guess0.png" % ifile, dpi = 400, bbox_inches = 'tight')

        print "Refining solution to 25 pixels..."
        (ibest, jbest) = roughastro(xref[flagref <= 4], x[flag <= 4] / npix, yref[flagref <=4], y[flag <= 4] / npix, ibest - 300, ibest + 300, jbest - 600, jbest + 600, 25)
        ...
        if doplot:
            ...
            plt.savefig("TMP/%04i_guess1.png" % ifile, dpi = 400, bbox_inches = 'tight')
        ...
        if doplot:
            ...
            plt.savefig("TMP/%04i_xy_match_DECam_o%i.png" % (ifile, order))
    
            # plot intersection in DECam coordinates before projection
            fig, ax = plt.subplots()
            ax.scatter(x1new - (x2new / npix + ibest), y1new - (y2new / npix + jbest), marker = 'o', facecolors = 'none', c = 'b') 
            plt.savefig("TMP/%04i_xy_diff_DECam_o%i.png" % (ifile, order))
        ...
        if doplot:
            ...
            plt.savefig("TMP/%04i_xy_match1_DuPont_o%i.png" % (ifile, order))
        ...
        if doplot:
            ...
            ax.scatter(x1t[mask] - x2new[mask], y1t[mask] - y2new[mask], marker = 'o', c = 'b', facecolors = 'none')
            plt.savefig("TMP/%04i_xy_diff1_DuPont_o%i.png" % (ifile, order))
    
            fig, ax = plt.subplots()
            ax.scatter(x1t[mask] - x2new[mask], y1t[mask] - y2new[mask], marker = '.', c = 'r')
        ...
        # plot new solution
        if doplot:
            ax.scatter(x1t[mask] - x2new[mask], y1t[mask] - y2new[mask], marker = 'o', c = 'g', facecolors = 'none')
            plt.savefig("TMP/%04i_xy_diff2_DuPont_o%i.png" % (ifile, order))
        ...
        if doplot:
            ...
            plt.savefig("TMP/%04i_xy_transformed_DuPont_o%i.png" % (ifile, order))
        ...
        fitsout = "%s/%s_%s_%04i_background_DECam_o%i.fits" % (outdir, supernova, filter, ifile, order)
        ...
        fitsout = "%s/%s_%s_%04i_DECam_o%i.fits" % (outdir, supernova, filter, ifile, order)
        proj.writeto(fitsout, clobber = True)
        ...
        if docrblaster:
            ...
            inmosaic = "%s/%s_%s_%04i_DECam_o%i.fits" % (outdir, supernova, filter, ifile, order)
            outmosaic = "%s/%s_%s_%04i_DECam_crblaster_o%i.fits" % (outdir, supernova, filter, ifile, order)
            backgroundfile = "%s/%s_%s_%04i_background_DECam_o%i.fits" % (outdir, supernova, filter, ifile, order)
    ...
    fitsout = "%s/%s_%s_nosky_DECam_o%i.fits" % (outdir, supernova, filter, order)
    proj.writeto(fitsout, clobber = True)
    ...
    fitsout = "%s/%s_%s_background_DECam_o%i.fits" % (outdir, supernova, filter, order)
    proj.writeto(fitsout, clobber = True)

    # save number of images in the mosaic
    fitsout = "%s/%s_%s_nmosaic_DECam_o%i.fits" % (outdir, supernova, filter, order)
...
if doconvolve:
    ...
    filename = "%s/%s_%s_nosky_DECam_o%i.fits" % (outdir, supernova, filter, order)
    ...
    filenmosaic = "%s/%s_%s_nmosaic_DECam_o%i.fits" % (outdir, supernova, filter, order)
